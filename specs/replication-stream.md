...
transdb-server/src/
  ...
  replication/service.rs  # ReplicationReceiver (replica)
```
...
message ReplicateBatchResponse { uint64 last_seq = 1; }
...
4. On success: drops all entries with `seq ≤ last_seq`.
...
`ReplicationReceiver` holds `db: Db` and implements the `Replication` service. The handler iterates entries in ascending `seq` order and for each acquires a write lock and applies:
...
`Server::run` binds the gRPC listener when `role == Replica && grpc_addr.is_some()`, spawns `ReplicationReceiver`, then starts the HTTP listener and sends `ready_tx`.
...
| **ACK**         | `last_seq` confirms all entries up to that seq; lower entries are safe to drop. |
...
| Write-lock timeout               | —                                             | `Status::unavailable`; primary retries                         |
| Empty batch                      | —                                             | `last_seq: 0`; no-op                                    |
...
**`ReplicationReceiver`:** single PutOp inserts and returns correct `last_seq`; PutOp overwrites unconditionally; DeleteOp writes tombstone for a present key; DeleteOp writes tombstone when key is absent; mixed batch applies in order; empty batch returns `last_seq: 0`; missing `operation` returns `invalid_argument`.
...
3. `feat(server): implement ReplicationReceiver (replica gRPC server)`
